...
function requireSetCookie() {
	...
	function parse(input, options) {
		...
		const cookies = {};
		return input.filter(isNonEmptyString).reduce((cookies, str) => {
...
const setCookieExports = /*@__PURE__*/ requireSetCookie();
		...
		start(controller) {
			if (body_size_limit !== undefined && content_length > body_size_limit) {
				...
				if (body_size_limit === 0) {
					...
					message +=
						" To disable body size limits, specify Infinity rather than 0.";
				}
...
async function getRequest({ request, base, bodySizeLimit }) {
	...
	if (request.httpVersionMajor >= 2) {
		...
		headers[":path"] = undefined;
		headers[":scheme"] = undefined;
	}
