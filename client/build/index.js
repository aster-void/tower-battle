...
class Trouter {
	...
	use(route, ...fns) {
		...
		const { keys, pattern } = parse$1(route, true);
		this.routes.push({ keys, pattern, method: "", handlers, midx: MAP[""] });
		return this;
	}

	add(method, route, ...fns) {
		const { keys, pattern } = parse$1(route);
		const handlers = [].concat.apply([], fns);
...
class Polka extends Trouter {
	...
	handler(req, res, next) {
		...
		const loop = async () =>
			res.finished || (i < len && arr[i++](req, res, next));
		(next =
			next ||
			((err) =>
				err ? this.onError(err, req, res, next) : loop().catch(next)))(); // init
	}
}
